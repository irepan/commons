...
public class DynamicTilesViewProcessor {
	...
	private static final Logger logger = Logger.getLogger(DynamicTilesViewProcessor.class);
	...
	protected void renderMergedOutputModel(String beanName, String url,
        ...
        container.render(definitionName, getRequest(container,request,response));
	...
	protected String startDynamicDefinition(String beanName, String url,
       ...
       String definitionName = processTilesDefinitionName(beanName, container,
        ...
        if (!definitionName.equals(beanName)) {
            ...
            AttributeContext attributeContext = container.startContext(getRequest(container,request,response));
	...
	protected void endDynamicDefinition(String definitionName, String beanName,
	                                    ...
	                                    TilesContainer container) {
        if (!definitionName.equals(beanName)) {
            container.endContext(getRequest(container,request,response));
        }
	}
	...
	protected String processTilesDefinitionName(String beanName,
		...
		if (derivedDefinitionName != null) {
		...
		} else if (container.isValidDefinition(beanName, getRequest(container,request,response))) {
			...
			if (container.isValidDefinition(sb.toString(), getRequest(container,request,response))) {
				result = sb.toString();
				...
				if (container.isValidDefinition(root, getRequest(container,request,response))) {
					result = root;
				} else {
    ...
    private Request getRequest(TilesContainer container, HttpServletRequest request,HttpServletResponse response){
    	return new org.apache.tiles.request.servlet.ServletRequest(container.getApplicationContext(),request,response);
    }
}
